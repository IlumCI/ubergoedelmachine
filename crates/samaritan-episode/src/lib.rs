...
// `extract_answer` now lives in samaritan-corpus, beside the grader that
// consumes its output: extraction and grading are two halves of one
// contract, and splitting them across crates meant anything wanting to
// score a reply had to depend on the whole episode stack. Re-exported here
// so every existing caller keeps working unchanged.
pub use samaritan_corpus::extract_answer;
